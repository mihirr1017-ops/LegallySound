// lib/keyManager.js
// ─────────────────────────────────────────────────────────────────────────────
// Supports two AI providers: OpenRouter (Qwen) and Mistral AI.
...
//
// Strategy:
//   1. Try all OpenRouter keys in order (cheapest / free tier first)
//   2. If all OpenRouter keys are exhausted, fall through to Mistral keys
//   3. If all Mistral keys are exhausted, throw a clear error
//
...
async function tryProvider(keys, callFn, providerName, messages, maxTokens, temperature) {
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    console.log(`[keyManager] ${providerName} key ${i + 1}/${keys.length} (${key.slice(0, 14)}...)`);
...
export async function callWithKeyRotation(messages, maxTokens = 600, temperature = 0.7) {
  ...
  if (orKeys.length > 0) {
    try {
      ...
      if (err.allExhausted) {
        console.log("[keyManager] All OpenRouter keys exhausted — falling through to Mistral");
      } else {
        throw err; // hard error, don't fall through
      }
    }
  }

  // ── Step 2: Try Mistral keys ────────────────────────────────────────────
  if (mistralKeys.length > 0) {
    try {
      ...
      if (err.allExhausted) {
        throw new Error(
          `All API keys exhausted (tried ${orKeys.length} OpenRouter + ${mistralKeys.length} Mistral key(s)). ` +
          `Add more keys to OPENROUTER_KEYS or MISTRAL_KEYS in Vercel environment variables.`
        );
      }
      throw err;
    }
